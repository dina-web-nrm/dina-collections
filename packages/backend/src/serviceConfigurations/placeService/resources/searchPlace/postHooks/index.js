...
const log = createLog('services/placeService/searchPlace/postHooks')
...
const extractIdsFromJobs = ({ jobs }) => {
  ...
  return jobs.reduce((ids, job) => {
    const jobPlaceIds = objectPath.get(
      job,
      'attributes.operationRequest.body.data.attributes.ids'
    )
    log.debug(
      `Found the following ids in job with id: ${job.id}: [${jobPlaceIds.join(
        ', '
      )}]`
    )
    if (jobPlaceIds && jobPlaceIds.length) {
      return [...ids, ...jobPlaceIds]
    }
    return ids
  }, [])
}
...
const consolidateJobsPostHook = ({ serviceInteractor, request }) => {
  ...
  const filter = {
    deactivatedAt: 'null',
    failedAt: 'null',
    includeOperationIds: ['searchPlaceRebuildView', 'searchPlaceUpdateView'],
    startedAt: 'null',
    succeededAt: 'null',
  }
    ...
    .then(({ data }) => {
      ...
      return Promise.resolve()
        .then(() => {
          ...
          return createIndexJob({
            searchPlaceIds: extractIdsFromJobs({ jobs: jobsToConsolidate }),
            serviceInteractor,
          })
        })
