describe('resourceManager: for each resource', () => {
  ...
  it(`shows table without errors and downloads CSV export`, () => {
    const testTableExport = ({ path }) => {
      ...
      cy.getByTestId('downloadButton', { timeout: 120000 }).should(
        'have.text',
        'Download'
      )
      cy.quickQueryByTestId('exportFailedMessage').should('not.exist')
  ...
  it(`creates, edits, deletes record for each resource except specimen and taxon`, () => {
    const testCreateEditDelete = ({ inputs, path } = {}) => {
      ...
      inputs.forEach(({ inputLabelFor, inputValue, optionText }) => {
        ...
        if (optionText) {
          cy.getDropdownOptionByText(optionText).click({ force: true })
        }
      })
